...
def parse_args():
    ...
    parser.add_argument('-s', '--sensor', type=int,
                        default=0, help="Camera sensor input (default 0")
...
def main():
    ...
    if sensor == 0:
        start_cam(args, sensor, shared, cls_dict, WINDOW_NAME.format(sensor))

    if sensor == 1 and show_cam == False:
        condition = threading.Condition()
        cam = Camera(args, sensor_id=sensor)
        if not cam.isOpened():
            raise SystemExit('ERROR: failed to open camera!')
